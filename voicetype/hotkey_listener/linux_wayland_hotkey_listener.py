...
class LinuxWaylandHotkeyListener(HotkeyListener):
    ...
    def __init__(
        ...
        self._action_id: Optional[int] = (
            None  # Store the action ID from GrabAccelerator
        )
        ...
        combined_desktop_info = (
            f"{xdg_current}:{original_xdg_current}:{gnome_session_mode}".lower()
        )
        ...
        if not is_gnome:
            ...
            is_kde = any(
                "kde" in part for part in xdg_current.lower().split(":") if part
            )
        ...
        if is_gnome:
            ...
            self._de_detected = (
                xdg_current.lower().split(":")[0] if xdg_current else "unknown"
            )
            logger.warning(
    ...
    def set_hotkey(self, hotkey: str) -> None:
        ...
        if not (hotkey.startswith("<") and hotkey.endswith(">")):
            logger.warning(
                f"Hotkey '{hotkey}' might not be in the expected D-Bus format (e.g., '<Control><Alt>X')."
            )
        self._hotkey = hotkey
    ...
    def _dbus_signal_handler(self, action_id: int, parameters: dict):
        ...
        logger.debug(
            f"D-Bus AcceleratorActivated signal received: action_id={action_id}, params={parameters}"
        )
        ...
        if self._action_id is not None and action_id == self._action_id:
            ...
            logger.debug(
                f"Ignoring signal for unknown or mismatched action_id: {action_id} (expected {self._action_id})"
            )
        ...
        try:
            ...
            if self._de_detected == "gnome":
                ...
                self._proxy = self._bus.get_proxy(
                    GNOME_SHELL_SERVICE, GNOME_SHELL_OBJECT_PATH
                )
                ...
                logger.info(
                    f"Connected to D-Bus signal: {signal_interface}.{signal_name}"
                )
                ...
                grab_method = getattr(
                    self._proxy, "GrabAccelerator"
                )  # TODO: FIX I think this is a permissions issue currently.
                # GrabAccelerator returns the action_id (uint32)
                self._action_id = grab_method(
                    self._hotkey, 0, 0
                )  # Flags=0, ModeFlags=0
                if self._action_id is None or self._action_id == 0:
                    logger.error(
                        f"Failed to grab accelerator '{self._hotkey}'. GrabAccelerator returned {self._action_id}."
                    )
                    # Handle error - maybe raise exception or disconnect?
                    raise RuntimeError(
                        f"Failed to register hotkey '{self._hotkey}' with GNOME Shell."
                    )
                else:
                    logger.info(
                        f"Registered accelerator with GNOME Shell: '{self._hotkey}' -> action_id={self._action_id}"
                    )
                ...
                logger.error(
                    f"Cannot register hotkey: Unsupported DE '{self._de_detected}'"
                )
            ...
            logger.error(
                "Failed to import 'dasbus'. Please install it: pip install dasbus",
                exc_info=True,
            )
    ...
    def start_listening(self) -> None:
        ...
        if self._de_detected != "gnome":
            ...
            logger.error(
                f"Cannot start Wayland listener: Only GNOME D-Bus is implemented (Detected: {self._de_detected})."
            )
            raise NotImplementedError(
                f"Wayland support only implemented for GNOME (Detected: {self._de_detected})."
            )
        ...
        self._loop_thread = threading.Thread(
            target=self._run_dbus_loop, name="DBusLoopThread", daemon=True
        )
        self._loop_thread.start()

    def stop_listening(self) -> None:
        """Stops the hotkey listener and cleans up D-Bus resources."""
        if not self._is_listening and (
        ...
        if self._proxy and self._de_detected == "gnome" and self._action_id is not None:
            try:
                ...
                logger.info(
                    f"Unregistered accelerator action_id={self._action_id} ('{self._hotkey}') from GNOME Shell."
                )
                self._action_id = None  # Clear the stored ID
            except Exception as e:
                # Log error but continue cleanup
                logger.error(
                    f"Error unregistering D-Bus accelerator action_id={self._action_id}: {e}"
                )
        ...
        if self._loop_thread and self._loop_thread.is_alive():
            ...
            if self._loop_thread.is_alive():
                logger.warning(
                    "D-Bus listener thread did not exit cleanly after timeout."
                )
    ...
    def __del__(self):
        ...
        if self._is_listening or (
            ...
            logger.warning(
                "HotkeyListener object deleted without explicit stop. Attempting cleanup..."
            )
            self.stop_listening()
